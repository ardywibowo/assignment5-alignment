...
def tokenize_prompt_and_output(
    ...
    prompt_ids = tokenizer(prompt_strs, add_special_tokens=False, padding=False)[
        "input_ids"
    ]
    output_ids = tokenizer(output_strs, add_special_tokens=False, padding=False)[
        "input_ids"
    ]
    ...
    for prompt, output in zip(prompt_ids, output_ids):
        ...
        response_mask = (
            [0] * len(prompt) + [1] * len(output) + [0] * (max_seq_len - len(combined))
        )
...
def get_response_log_probs(
    ...
    log_probs_gathered = torch.gather(log_probs, -1, labels.unsqueeze(-1)).squeeze(
        -1
    )  # (batch_size, seq_len)
...
def sft_microbatch_train_step(
    ...
    loss = -sum_log_probs / (
        batch_size * gradient_accumulation_steps * normalize_constant
    )
